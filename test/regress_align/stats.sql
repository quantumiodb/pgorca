...
EXPLAIN (COSTS ON) SELECT count(*) FROM test_last_scan WHERE noidx_col = 1;
SELECT count(*) FROM test_last_scan WHERE noidx_col = 1;
SET LOCAL enable_seqscan TO off;
EXPLAIN (COSTS ON) SELECT count(*) FROM test_last_scan WHERE idx_col = 1;
...
EXPLAIN (COSTS ON) SELECT COUNT(*) FROM test_io_shared t1 INNER JOIN test_io_shared t2 USING (a);
...
EXPLAIN (COSTS ON) SELECT * FROM brin_hot_2 WHERE a = 2 AND b = 100;
...
EXPLAIN (COSTS ON) SELECT * FROM brin_hot_3 WHERE a = 2;
