...
EXPLAIN (costs ON) UPDATE rw_view1 SET a=6 WHERE a=5;
EXPLAIN (costs ON) DELETE FROM rw_view1 WHERE a=5;
...
EXPLAIN (costs ON) UPDATE rw_view2 SET aaa=5 WHERE aaa=4;
EXPLAIN (costs ON) DELETE FROM rw_view2 WHERE aaa=4;
...
EXPLAIN (costs ON) UPDATE rw_view2 SET a=3 WHERE a=2;
EXPLAIN (costs ON) DELETE FROM rw_view2 WHERE a=2;
...
EXPLAIN (verbose, costs ON) UPDATE rw_view1 SET b = b + 1 RETURNING *;
...
EXPLAIN (costs ON) INSERT INTO rw_view1 VALUES (5);
EXPLAIN (costs ON) UPDATE rw_view1 SET a = a + 5;
...
EXPLAIN (costs ON) SELECT * FROM rw_view1 WHERE snoop(person);
EXPLAIN (costs ON) UPDATE rw_view1 SET person=person WHERE snoop(person);
EXPLAIN (costs ON) DELETE FROM rw_view1 WHERE NOT snoop(person);
EXPLAIN (costs ON)
MERGE INTO rw_view1 t
  USING (VALUES ('Tom'), ('Dick'), ('Harry')) AS v(person) ON t.person = v.person
  WHEN MATCHED AND snoop(t.person) THEN UPDATE SET person = v.person;
...
EXPLAIN (costs ON) SELECT * FROM rw_view2 WHERE snoop(person);
EXPLAIN (costs ON) UPDATE rw_view2 SET person=person WHERE snoop(person);
EXPLAIN (costs ON) DELETE FROM rw_view2 WHERE NOT snoop(person);
EXPLAIN (costs ON)
MERGE INTO rw_view2 t
  USING (VALUES ('Tom'), ('Dick'), ('Harry')) AS v(person) ON t.person = v.person
  WHEN MATCHED AND snoop(t.person) THEN UPDATE SET person = v.person;
...
EXPLAIN (costs ON) DELETE FROM rw_view1 WHERE id = 1 AND snoop(data);
DELETE FROM rw_view1 WHERE id = 1 AND snoop(data);

EXPLAIN (costs ON) INSERT INTO rw_view1 VALUES (2, 'New row 2');
...
EXPLAIN (COSTS ON) UPDATE v2 SET a = 1;
