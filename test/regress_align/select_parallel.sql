...
explain (costs ON) execute tenk1_count(1);
...
reset enable_seqscan;
alter table tenk2 set (parallel_workers = 0);
explain (analyze, timing off, summary off, costs ON, buffers off)
...
SAVEPOINT settings;
SET LOCAL debug_parallel_query = 1;
EXPLAIN (analyze, timing off, summary off, costs ON, buffers off) SELECT * FROM tenk1;
ROLLBACK TO SAVEPOINT settings;
...
EXPLAIN (COSTS ON) EXECUTE pstmt('1', make_some_array(1,2));
