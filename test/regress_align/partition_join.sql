...
EXPLAIN (COSTS ON) -- Should use NestLoop with parameterised inner scan
...
EXPLAIN (COSTS ON) SELECT * FROM pht1 p1 JOIN pht1 p2 USING (c) LIMIT 1;
-- Increase number of tuples requested and an IndexScan will be chosen
EXPLAIN (COSTS ON) SELECT * FROM pht1 p1 JOIN pht1 p2 USING (c) LIMIT 100;
-- If almost all the data should be fetched - prefer SeqScan
EXPLAIN (COSTS ON) SELECT * FROM pht1 p1 JOIN pht1 p2 USING (c) LIMIT 1000;
