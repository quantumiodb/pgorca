...
EXPLAIN (COSTS ON, VERBOSE) SELECT * FROM jsonb_table_view2;
EXPLAIN (COSTS ON, VERBOSE) SELECT * FROM jsonb_table_view3;
EXPLAIN (COSTS ON, VERBOSE) SELECT * FROM jsonb_table_view4;
EXPLAIN (COSTS ON, VERBOSE) SELECT * FROM jsonb_table_view5;
EXPLAIN (COSTS ON, VERBOSE) SELECT * FROM jsonb_table_view6;
...
EXPLAIN (COSTS ON, FORMAT JSON, VERBOSE)
SELECT * FROM
	JSON_TABLE(
		jsonb 'null', 'lax $[*]' PASSING 1 + 2 AS a, json '"foo"' AS "b c"
		COLUMNS (
