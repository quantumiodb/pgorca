...
begin;
explain (costs ON) declare c1 cursor for select (select 42) as x;
explain (costs ON) declare c1 scroll cursor for select (select 42) as x;
declare c1 scroll cursor for select (select 42) as x;
fetch all in c1;
fetch backward all in c1;
rollback;
begin;
explain (costs ON) declare c2 cursor for select generate_series(1,3) as g;
explain (costs ON) declare c2 scroll cursor for select generate_series(1,3) as g;
