...
TPC-H Q15 在 pg_orca 中触发 fallback，根本原因是 DXL 翻译层遇到 CTE 节点时直接抛出异常：
...
DXL 子节点翻译顺序：Sequence 先翻译 Producer（child[1]），后翻译包含 Consumer 的主计划（child[2..N]）。因此 Producer 翻译时先调用 `AllocCTEParamId`，Consumer 翻译时查到的 `param_id` 已存在。
...
- 遍历 projlist 构建 custom_scan_tlist 和 plan->targetlist：
    ...
    collid   = gpdb::TypeCollation(type_oid)
...
| Producer Rescan | 清除 tuplestore，重置 PARAM_EXEC 槽，重新驱动子计划 |
| tuplestore 生命周期 | Producer EndCustomScan 调用 `tuplestore_end`；Consumer EndCustomScan 清理自身读指针状态 |
| 错误中止 | PARAM_EXEC 槽随 EState 一起释放；tuplestore 在 executor memory context 释放 |
