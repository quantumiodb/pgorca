...
//
//	Implementation of left anti-semi index nested-loops join operator
//---------------------------------------------------------------------------
...
CPhysicalLeftAntiSemiIndexNLJoin::CPhysicalLeftAntiSemiIndexNLJoin(
...
BOOL
CPhysicalLeftAntiSemiIndexNLJoin::Matches(COperator *pop) const
{
	if (pop->Eopid() == Eopid())
	{
		return m_pdrgpcrOuterRefs->Equals(
			CPhysicalLeftAntiSemiIndexNLJoin::PopConvert(pop)
				->PdrgPcrOuterRefs());
	}
...
CDistributionSpec *
CPhysicalLeftAntiSemiIndexNLJoin::PdsRequired(
	...
	CDrvdPropArray *pdrgpdpCtxt GPOS_UNUSED, ULONG) const
{
	GPOS_RAISE(CException::ExmaInvalid, CException::ExmiInvalid,
			   GPOS_WSZ_LIT("PdsRequired should not be called for "
...
CEnfdDistribution *
CPhysicalLeftAntiSemiIndexNLJoin::Ped(CMemoryPool *mp,
